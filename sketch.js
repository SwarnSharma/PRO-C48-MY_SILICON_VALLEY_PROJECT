...
var LEVEL1=0;
var LEVEL2=1;
var loading1,loadingImg1;
var Img; 
var img1,img2,img3,img4;

var ground, upperGround, restGround;
var groundTile,groundTileImg;
var wall, wallImg;
...
function preload(){
  loadingImg1 = loadImage("Images/Loading.gif");
  ...
  groundTileImg = loadImage("Images/Ground_tile.png");
  wallImg = loadImage("Images/wall.png");
  lift1Img = loadImage("Images/lift.png");
...
function setup() {
  ...
  //player = createSprite(1240, 560, 30, 60);
 ...
 var treasure=createSprite(2550, 560, 10, 10);
 treasure.addImage(treasureImg);
 treasure.scale=0.5

 lava = createSprite(1950, 1105, 960, 19);
 lava.shapeColor="red";
 ...
 restGround = createSprite(2500, 1090, 200, 40);
  ...
  lift1.velocityY = -1;
  lift1.addImage(lift2Img);
  lift1.scale=1.5
  //lift1.debug=true;

  lift2 = createSprite(250, 500, 70, 10);
  lift2.velocityY = -1;
  lift2.addImage(lift2Img);
  lift2.scale=1.5
  //lift2.debug=true;
  ...
  // loading1 = createSprite(windowWidth/2,windowHeight/2,windowWidth,windowHeight);
  // loading1.addImage(loadingImg1);
  // loading1.scale= 1.8

  for (var o =-118; o<1236; o=o+36){
    for(var p=567; p<650; p=p+36){
    groundTile=createSprite(o,p,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.03
    }
  }
  for (var q =332; q<1370; q=q+27){
    for(var r=348.2; r<380; r=r+25){
    groundTile=createSprite(q,r,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.022
    }
  }

  for(var s=-115; s<5; s=s+42){
    for(var t=84; t<560; t=t+62){
    wall=createSprite(s,t,10,10);
    wall.addImage(wallImg);
    wall.scale=0.4
    }
  }
  for(var u=31; u<2600; u=u+42){
    for(var v=114; v<150; v=v+62){
    wall=createSprite(u,v,10,10);
    wall.addImage(wallImg);
    wall.scale=0.4
    }
  }
  for(var w=1396; w<2630; w=w+42){
    for(var x=114; x<430; x=x+62){
    wall=createSprite(w,x,10,10);
    wall.addImage(wallImg);
    wall.scale=0.4
    }
  }
  for(var aa=1396; aa<1400; aa=aa+42){
    for(var bb=486;bb<600; bb=bb+62){
    wall=createSprite(aa,bb,10,10);
    wall.addImage(wallImg);
    wall.scale=0.4
    }
  }
  for(var cc=1250; cc<1400; cc=cc+36){
    for(var dd=568;dd<640; dd=dd+36){
    groundTile=createSprite(cc,dd,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.03
    }
  }
  ...
  var spot=createSprite(1420,565,16,32);
  spot.shapeColor="black";
  ...
  for(var ee=1000; ee<1220; ee=ee+36){
    for(var ff=676;ff<1150; ff=ff+36){
    groundTile=createSprite(ee,ff,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.03
    }
  }
  for(var gg=1252; gg<2630; gg=gg+36){
    for(var hh=1126;hh<1150; hh=hh+36){
    groundTile=createSprite(gg,hh,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.03
    }
  }
  for(var ii=1252; ii<1480; ii=ii+36){
    for(var jj=1090;jj<1100; jj=jj+36){
    groundTile=createSprite(ii,jj,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.03
    }
  }
  for(var kk=2419; kk<2650; kk=kk+36){
    for(var ll=1090;ll<1100; ll=ll+36){
    groundTile=createSprite(kk,ll,10,10);
    groundTile.addImage(groundTileImg);
    groundTile.scale=0.03
    }
  }
...
function draw() {
  ...
  if(i===true){
    ...
    if(player.collide(ground) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(lift1) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(restGround) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(lift3) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(lift4) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(lift5) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block1) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block2) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block3) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block4) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block5) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block6) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block7) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(block8) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(ground2) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(lift2) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    if(player.collide(box1) && h==true ){
      player.addAnimation("idle",playerStanding);
      h=false;
    }
    }
    ...
    if(keyDown("space") && g==false  && player.y>145 && (player.collide(ground) || player.collide(restGround) || player.collide(lift3) || player.collide(lift4) || player.collide(lift5) || player.collide(block1) || player.collide(block2) || player.collide(block3) || player.collide(block4) || player.collide(block5) || player.collide(block6) || player.collide(block7) || player.collide(block8) || player.collide(ground2) || player.collide(lift1) || player.collide(lift2) || player.collide(box1))) {
     ...
     if(player.collide(ground) && g==true){ 
      player.addAnimation("idle",playerStanding2);
      g=false;
     }
     if(player.collide(lift1) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(restGround) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(lift3) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(lift4) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(lift5) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block1) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block2) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block3) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block4) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block5) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block6) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block7) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(block8) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(ground2) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(lift2) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
    if(player.collide(box1) && g==true ){
      player.addAnimation("idle",playerStanding2);
      g=false;
    }
